...
namespace App\Components\Competition\Validation;

use Intervention\Image\Image;

class CompetitionValidate
{

    public static function validate()
    {
        $storage_path = STORAGE_PATH . DS . 'competitions';
        ...
        if (! empty($image_thumb['tmp_name'])) {
            ...
            if (! $result['image_thumb']) {
                $result['image_thumb'] =
                    sprintf('%s_thumb.%s', md5($result['title']), explode('/', $image_thumb['type'])[1]);
            }
